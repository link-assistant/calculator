...
Research and behavior were checked on 2026-09-15. The umbrella issue and all
three child issues were read in full; none of issues 217-220 had comments at
...
| --- | --- | --- | --- |
| R1 | #220 | Read and implement every requirement in child issues #217, #218, and #219, including comments. | All four issue bodies and their complete comment feeds were reviewed. The concrete bugs are covered by issue-specific integration tests; the market request is covered by a sourced audit, compatibility suite, natural-language aliases, and a prioritized gap plan. |
...
| R9 | #218 | Add competitors' published examples so compatible expressions remain working. | `issue_218_competitor_compatibility_tests.rs` turns the deterministic, currently supported seed examples into executable tests and records unsupported examples rather than claiming false parity. |
| R10 | #218 | Expand natural-language expression coverage. | Documented Numi/Parsify aliases for addition, subtraction, multiplication, division, and modulo are now lexer/parser syntax, including `multiplied by` and `divided by`. |
...
Option 3 was selected. The first implementation slice adds the natural word
operators shared by Numi and Parsify. The audit is deliberately explicit about
unsupported examples and semantic conflicts, so future work can move rows from
“gap” to the executable corpus without overstating compatibility.
...
- The compatibility suite exercises public `Calculator` behavior instead of an
  internal helper, providing an end-to-end contract.
